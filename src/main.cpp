...
// const unsigned long AUTO_SLEEP_TIME = 120000;    // Auto sleep หลัง 2 นาที (DISABLED)
...
void sendToGistNorth() {
  ...
  if (https.begin(client, url)) {
    ...
    if (pmsConnected && dataReady) {
      doc["pm25"] = (float)pmsData.pm2_5_atm;
      doc["pm10"] = (float)pmsData.pm10_atm;
    ...
    Serial.printf("[%s] SEND -> %s | pm25=%.1f pm10=%.1f | ",
...
void updateDisplay() {
  ...
  if (!pmsConnected) {
    ...
    sprite.setTextFont(2);
    sprite.setTextColor(color, TFT_BLACK);
    sprite.drawString(getAirQuality(pmsData.pm2_5_atm), 310, 90);
  ...
  if (lowBatteryWarn) {
    ...
    sprite.setTextColor(TFT_CYAN, TFT_BLACK);
    sprite.drawString("Always On", 315, 25);
  }
  
  // Auto sleep countdown removed - system runs continuously
...
void setup() {
  ...
  // Automatically activate system on boot (no sleep, always on)
  systemActive = true;
  displayOn = true;
...
void loop() {
  ...
  if (systemActive) {
    // Auto sleep disabled - system runs continuously
